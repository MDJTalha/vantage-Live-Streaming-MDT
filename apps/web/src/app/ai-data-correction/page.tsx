...
import { Sparkles, Database, Brain, CheckCircle, AlertTriangle, Search } from 'lucide-react';
...
export default function AIDataCorrection() {
  ...
  const runDataAudit = async () => {
    setIsRunning(true);
    try {
      const response = await fetch('/api/v1/ai/audit-quality', {
        method: 'GET',
        headers: {
  ...
  const correctMeetingData = async (meetingId: string) => {
    try {
      const response = await fetch(`/api/v1/ai/correct-meeting/${meetingId}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
  ...
  const enhanceRecordings = async () => {
    try {
      const response = await fetch('/api/v1/ai/enhance-recordings', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
        },
  ...
  const buildKnowledgeGraph = async () => {
    try {
      const response = await fetch('/api/v1/ai/build-knowledge', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
        },
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-3xl font-bold text-white">AI Data Correction</h1>
          </div>
